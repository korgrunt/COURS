...
void my_body(unsigned char *candidate, __m128i *res, unsigned char *target)
{
  ...
  __m128i targetHash = _mm_set_epi32(target[3], target[2], target[1], target[0]);
  __m128i cmpResult = _mm_cmpeq_epi32(res[0], targetHash);
  cmpResult = _mm_and_si128(cmpResult, _mm_cmpeq_epi32(res[1], targetHash));
  cmpResult = _mm_and_si128(cmpResult, _mm_cmpeq_epi32(res[2], targetHash));
  cmpResult = _mm_and_si128(cmpResult, _mm_cmpeq_epi32(res[3], targetHash));

  // Check if all components are equal
  int wasEquals = _mm_movemask_epi8(cmpResult) == 0xFFFF;

  if (wasEquals) {
      // Print the correct candidate
      printf("Candidate: %s\n", candidate);
